...
function dropsToXrp(dropsToConvert) {
    let drops;
    try {
        drops = new bignumber_js_1.default(dropsToConvert).toString(BASE_TEN);
    }
    catch (_err) {
        throw new errors_1.ValidationError(`dropsToXrp: invalid value '${typeof dropsToConvert === 'string' ? dropsToConvert : JSON.stringify(dropsToConvert)}', should be a BigNumber or string-encoded number.`);
    }
    if (drops.includes('.')) {
...
function xrpToDrops(xrpToConvert) {
    let xrp;
    try {
        xrp = new bignumber_js_1.default(xrpToConvert).toString(BASE_TEN);
    }
    catch (_err) {
        throw new errors_1.ValidationError(`xrpToDrops: invalid value '${typeof xrpToConvert === 'string' ? xrpToConvert : JSON.stringify(xrpToConvert)}', should be a BigNumber or string-encoded number.`);
    }
