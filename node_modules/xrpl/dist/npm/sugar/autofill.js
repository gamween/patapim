...
function calculateSponsorFee(netFeeDrops, sponsorSignersCount = 0) {
    if (sponsorSignersCount <= 0) {
        return new bignumber_js_1.default(0);
    }
    console.warn(`For sponsored transaction the auto calculated Fee accounts for sponsor signers to avoid transaction failure.`);
    return new bignumber_js_1.default(scaleValue(netFeeDrops, sponsorSignersCount));
}
function calculateFeePerTransactionType(client, tx, signersCount = 0, sponsorSignersCount = 0) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (tx.TransactionType === 'EscrowFinish' && tx.Fulfillment != null) {
            ...
            baseFee = bignumber_js_1.default.sum(baseFee, scaleValue(netFeeDrops, CONFIDENTIAL_FEE_MULTIPLIER));
        }
        if (signersCount > 0) {
        ...
        baseFee = bignumber_js_1.default.sum(baseFee, sponsorFee);
...
function getTransactionFee(client, tx, signersCount = 0, sponsorSignersCount = 0) {
    return __awaiter(this, void 0, void 0, function* () {
        const fee = yield calculateFeePerTransactionType(client, tx, signersCount, sponsorSignersCount);
...
function checkAccountDeleteBlockers(client, tx) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const [objectsResponse, infoResponse] = yield Promise.all([
            client.request(objectsRequest),
            client.request(infoRequest),
        ]);
        if (objectsResponse.result.account_objects.length > 0) {
            throw new errors_1.XrplError(`Account ${tx.Account} cannot be deleted; there are Escrows, PayChannels, RippleStates, Checks, or Sponsorships associated with the account.`, objectsResponse.result.account_objects);
        }
        const accountRoot = infoResponse.result.account_data;
        if (accountRoot.SponsoringOwnerCount != null ||
            accountRoot.SponsoringAccountCount != null) {
            throw new errors_1.XrplError(`Account ${tx.Account} cannot be deleted; it has outstanding sponsorship obligations (SponsoringOwnerCount/SponsoringAccountCount).`);
        }
        if (accountRoot.Sponsor != null &&
            'Destination' in tx &&
            typeof tx.Destination === 'string' &&
            !(0, common_1.areAddressesEqual)(accountRoot.Sponsor, tx.Destination)) {
            throw new errors_1.XrplError(`Account ${tx.Account} cannot be deleted; its Sponsor (${accountRoot.Sponsor}) does not match the AccountDelete Destination (${tx.Destination}).`);
        }
    });
}
